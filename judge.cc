...
judge_c::judge_c(const string &compiler,const string &partdir,const string &probdir)
{
#ifdef __WIN32
	this->compiler=(compiler==""?string("g++ -O2 -o %TJ_EXEC% %TJ_SRC%"):compiler);
#else
	this->compiler=(compiler==""?string("g++ -O2 -o $TJ_EXEC $TJ_SRC"):compiler);
#endif
...
bool judge_c::compile(const string &compiler,const string &src,const string &exec,const string &log)
{
	//todo: time limit
...
bool judge_c::runjudge(string &glblog,bool nolog)
{
	...
	for(vector<string>::iterator curprob=probs.begin();curprob!=probs.end();curprob++)
	{
	...
	char buf[20];
	for(vector<string>::iterator curpart=parts.begin();curpart!=parts.end();curpart++)
	{
		int partscore=0;
		for(vector<string>::iterator curprob=probs.begin();curprob!=probs.end();curprob++)
		{
			...
			bool compileres;
			if(chdir(curpartdir.c_str())==-1)compileres=false;
			else compileres=compile(compiler,src,exec,log);
			...
			else
			{
				...
				for(int i=0;i<atoi(config[*curprob]["testcnt"].c_str());i++)
				{
					...
					loclog+=string("\ntest")+itoa(i,buf,10)+": ";
					file.setfile(curprobdir+itoa(i,buf,10)+config[*curprob]["insuf"]);
					...
					unlink((curpartdir+*curprob+config[*curprob]["outsuf"]).c_str());	//no verifications in case of lacking output file
					int runres;
					if(chdir(curpartdir.c_str())==-1)runres=-1;
					else runres=rlimit.run();
...
#endif
					file.setfile(curprobdir+itoa(i,buf,10)+config[*curprob]["anssuf"]);
					switch(file.comp(curpartdir+*curprob+config[*curprob]["outsuf"],
						config[*curprob]["bincomp"]=="true"?true:false))
					{
						...
						default:
							...
							log_out<<"wrong answer for test "<<itoa(i,buf,10)
								<<endl<<"***output***"<<endl;
			...
			if(!nolog)
			{
				...
				if(chdir(curpartdir.c_str())!=-1)
				{
					file.setfile(log);
					string logstr;
					if(file.readall(logstr))loclog+=logstr;
					else loclog+="(cannot fetch log)\n";
				}
				else loclog+="(cannot fetch log)\n";
